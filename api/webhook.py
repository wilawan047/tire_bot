...
def handle_message(event):
    ...
    try:
        ...
        # จัดการ Quick Reply เวลาเปิดทำการ
        if text == "เวลาเปิดทำการ":
            set_user_mode(user_id, "menu")
            line_bot_api.reply_message(
                reply_token,
                TextSendMessage(
        ...
        # จัดการคำถามเพิ่มเติม (เรียก Make)
            ...
            example_questions = [
                "ยางที่เหมาะกับรถเก๋ง?",
                "ยางที่เหมาะกับรถกระบะ?",
                "ยางที่เหมาะกับขับในเมือง?",
        ...
        if mode == "free_text":
            ...
            if text in ["แนะนำ", "เมนูหลัก"]:
            ...
            elif any(trigger in text for trigger in ["ยี่ห้อ", "รุ่น", "ติดต่อร้านยาง", "ติดต่อเรา", "เบอร์โทร", "โทรศัพท์", "เวลาเปิดทำการ", "บริการ", "โปร", "promotion", "โปรโมชัน", "service"]):
                # ถ้าพิมพ์คำสั่งนำทาง ให้เปลี่ยนกลับเป็นโหมดเมนู
                set_user_mode(user_id, "menu")
                ...
                try:
                    ...
                    if make_answer:
                        try:
                            # ส่งคำตอบจาก Make (มีข้อความออกจากเมนูรวมอยู่แล้ว)
