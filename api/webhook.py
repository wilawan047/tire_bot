...
def handle_message(event):
    ...
    try:
        ...
        # 🔟 ตัวอย่างคำถาม
            ...
            print("➡️ ผู้ใช้เลือกถามเพิ่มเติม แสดงตัวอย่างคำถาม")
            line_bot_api.reply_message(
                reply_token,
                TextSendMessage(
                    text=(
        ...
        # 🟢 ตรวจว่าผู้ใช้ถามตรงกับตัวอย่างไหม
        elif text in example_questions:
            print(f"➡️ ผู้ใช้ถามคำถามตัวอย่าง: {text} → ส่งไป Make เลย")
            reply_text = forward_to_make(text)
            line_bot_api.reply_message(
                reply_token,
